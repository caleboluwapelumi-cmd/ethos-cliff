...
export default function ContactForm() {
  ...
  if (submitted) {
    ...
    return (
      <div className="py-8 motion-safe:animate-fade-in">
        <p className="text-h2" style={{ fontSize: "clamp(1.75rem, 3vw, 2.5rem)" }}>
          Thanks {firstName}. We&rsquo;ll be in touch within 24 hours.
        </p>
        <p className="mt-4 text-body" style={{ fontSize: "1.05rem" }}>
          We&rsquo;ve received your enquiry and will reach out to you at{" "}
          <span style={{ color: "var(--ec-ink)" }}>{data.email}</span>{" "}
          shortly.
        </p>
      </div>
    );
  ...
  return (
    <div>
      {/* Step indicator */}
      <div className="mb-10 flex items-center gap-3" aria-label="Form progress">
          ...
          <span key={s} className="flex items-center gap-3">
            ...
            <span
              style={{
                ...
                fontSize: s === step ? "1rem" : "0.85rem",
                fontWeight: s === step ? 700 : 400,
                color: s === step ? "var(--ec-red)" : "var(--ec-ink-faint)",
      ...
      <div key={step} className="motion-safe:animate-fade-in space-y-8" style={{ animationDuration: "200ms" }}>
        ...
        {step === 2 && (
          <>
            <Field label="Service needed" id="service" required error={errors.service}>
              <select
                id="service"
                value={data.service}
        ...
        <div className="flex items-center justify-between gap-4 pt-2">
          {step > 1 ? (
            <button type="button" onClick={back} className="btn-text">
              &larr; Back
            </button>
          ) : (
            <div />
